...
var Peer = function (options) {
	...
	this.jsonrpc = new JsonRPC(options);
	...
	this.jsonrpc.once('open', function () {
		try {
			that._info().then(function (daemonInfo) {
				that.daemonInfo = daemonInfo;
			}).catch(function () {}).finally(function () {
				if (that.options.user) {
					that._authenticate(that.options.user, that.options.password).then(function (access) {
						that.access = access;
						that.connected = true;
 ...
 * Authenticate
 */
Peer.prototype._authenticate = function (user, password) {
 ...
 * Config
 */
Peer.prototype.config = function (params) {
	var jsonrpc = this.jsonrpc;
	return this.connectPromise.then(function () {
		return jsonrpc.service('config', params);
	});
};
